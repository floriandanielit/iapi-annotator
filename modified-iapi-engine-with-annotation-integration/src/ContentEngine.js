...
function injectIAPI() {
	...
	form_data.append("target_url", String(window.location.href));
			...
			if (xhr.status === 200) {  
				...
				if(xmlDoc) {					
					...
					for (var i=0; i < annotation.length; i++) {
						...
						console.log("iAPIvalue: " + iAPIvalue);
						
						$(CSSpath).addClass(iAPIvalue);
						
						if (iAPIvalue.substring(0, 4) == "iapi") {
							$(CSSpath).attr("id",CSSid);
						}
					}
				}
			}
    ...
    getStoredObject(function (data) {
        ...
        if (sourceType === "iapi") {
            ...
            $.each(data, function (key, value) {
				...
				// FUCK ME template.find("[class*='dataitem:']").css( "background-color", "red" );
                //get the dataitem
